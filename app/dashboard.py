...
st.set_page_config(
    page_title="Nigeria 2027 Election Predictor",
    page_icon="🗳️",
    layout="wide",
...
states_data = {
    ...
    'zone': ['South-East','North-East','South-South','South-East','North-East','South-South',
              ...
              'South-West','North-Central','South-South','North-West','North-East','North-East','North-West'],
    'base_apc_prob': [0.18, 0.42, 0.32, 0.12, 0.55, 0.58, 0.38, 0.65,
                      ...
                      0.58, 0.45, 0.68, 0.62],
    'population_m': [3.7, 4.2, 5.5, 6.0, 7.2, 2.3, 6.0, 5.8, 4.0, 5.6, 3.3, 4.7,
...
df['apc_prob'] = compute_probs(df, inflation_2027, approval_index,
                                opposition_united, oil_price, security_worsens)
...
with col1:
    st.metric("🔵 APC Win Probability", f"{apc_win_pct:.1f}%",
              delta=f"{'Favored ✅' if apc_win_pct > 50 else 'Underdog ⚠️'}")
with col2:
    st.metric("🟢 Opposition Win Prob.", f"{opp_win_pct:.1f}%",
              delta=f"{'Favored ✅' if opp_win_pct > 50 else 'Underdog ⚠️'}")
with col3:
...
with col_left:
    ...
    fig_states = px.bar(
        ...
        orientation='h',
        labels={'apc_prob': 'APC Win Probability', 'state': 'State', 'predicted_winner': 'Projected Winner'},
...
with col_right:
    ...
    fig_mc.add_trace(go.Histogram(
        x=vote_dist, nbinsx=60,
        marker_color='#2E86AB', opacity=0.8,
        name='APC Vote Share'
    ))
    fig_mc.add_vline(x=50, line_dash="dash", line_color="red",
                     annotation_text="50%", line_width=2)
    fig_mc.add_vline(x=vote_dist.mean(), line_dash="solid", line_color="orange",
                     annotation_text=f"Mean: {vote_dist.mean():.1f}%", line_width=2)
...
fig_zone = px.bar(zone_summary, x='zone', y=['apc_states', 'opp_states'],
                  color_discrete_map={'apc_states': '#003399', 'opp_states': '#008751'},
                  barmode='group',
                  labels={'value': 'Number of States', 'zone': 'Geopolitical Zone',
                          'variable': 'Party'},
...
# st.markdown("**Built by Daniel Diala | Le Wagon Data Science & AI Bootcamp | 2026** | [LinkedIn](https://www.linkedin.com/in/danieldiala/) | [GitHub](https://github.com/dd4real2k)")
